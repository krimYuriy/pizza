...
import { increasePizzaCount, decreasePizzaCount, removePizza, removePizzaItem } from '../redux/actions/cart'
import emptyCartImage from '../scss/img/empty-cart.png'

const Cart = () => {
   const { items, totalPizzasCount, totalPizzasPrice } = useSelector(({ cart }) => cart)


   const addedPizzaItems = Object.keys(items).map(key => {
      const length = items[key].pizzas.length
      return items[key].pizzas[length - 1]
   })

   const dispatch = useDispatch()

   const clickRemoveHandler = () => {
      if (window.confirm('Удалить все пиццы ?')) dispatch(removePizza())
   }
            ...
            totalPizzasCount
               ? <div className="cart">
                  ...
                  <div className="content__items">
                     {
                        addedPizzaItems.map(obj => {
                           return <CartItem
                              id={obj.id}
                              key={obj.id}
                              pizzaSum={items[obj.id].pizzaPrice}
                              pizzaCount={items[obj.id].pizzas.length}
                              type={obj.type}
                              size={obj.size}
                              image={obj.imageUrl}
                              name={obj.name}
                              removePizza={deletePizzaItem}
                              increasePizza={plusPizza}
                              decreasePizza={minusPizza}
                           />
                        })
                     }
                  </div>
                  <div className="cart__bottom">
                     <div className="cart__bottom-details">
                        ...
                        <span> Сумма заказа: <b>{totalPizzasPrice} ₽</b> </span>
                     </div>
